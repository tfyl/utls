...
// CertificateCompressionAlgorithm identifies a compression algorithm for use
// with the draft TLS Certificate Compression extension.
type CertificateCompressionAlgorithm uint16

const (
	CompressionZlib   CertificateCompressionAlgorithm = 1
	CompressionBrotli                                 = 2
)
...
func (m *compressedCertificateMsg) unmarshal(data []byte) bool {
	...
	m.algorithm = CertificateCompressionAlgorithm(algID)
